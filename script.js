...
document.addEventListener("DOMContentLoaded", () => {
    ...
    initLightbox();
    initSmoothScroll();
    initNavbarScroll(); // Jag lade till denna här!
});
...
function initLightbox() {
    ...
    lightbox.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(touchEndX - touchStartX) > 50) {
            if (touchEndX - touchStartX < 0) showNext();
            else showPrev();
        }
    });
}
...
// ===== Navbar scroll effect (Nu inuti en funktion!) =====
function initNavbarScroll() {
    const navbar = document.querySelector('.navbar');
    // Om navbar inte finns på denna sida (t.ex om du byter design), avbryt
    if (!navbar) return;
